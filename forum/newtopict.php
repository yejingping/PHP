...
if ($validforum==0) {
    ...
    $messagesql = "INSERT INTO messages(date,user_id,topic_id,subject,body)VALUES (NOW(),".$_SESSION['USERID'].",".mysqli_insert_id($link);",'".$_POST['subject']."','".$_POST['body']."')";
    ...
    if($validforum != 0 ){
        $namesql = "SELECT name From forums WHERE name=".."";
...
<form action="<?php echo pf_script_with_get($SCRIPT_NAME)?>" method="post">
    <div id="fill">
        ...
        if($validforum == 0) {
            ...
            $forumsresult = mysqli_query($link, $validforum);
            ?>
            <label>版块:</label>
            <select name="forum">
                <?php
                while ($foumsrow = mysqli_fetch_assoc($forumsresult)) {
